...
module Reader : READER = struct
  ...
  and nt_vector str =
    ...
    let nt3 = pack (caten nt3 (char ')')) (fun (sexprs, _) -> 
                                            ScmVector(sexprs)) in

    let nt2 = disj nt2 nt3 in
    
    let nt1 = pack (caten nt1 nt2) (fun (_,vector) -> vector) in
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
    ...
    | ScmPair (car, cdr) ->
      (* raise (X_not_yet_implemented "hw 1 ScmPair (car, cdr)") *)
  ...
  let beginify = function
    | ScmNil -> ScmVoid
    | ScmPair (e, ScmNil) -> e
    | e -> ScmPair (ScmSymbol "begin", e);;
  

  let rec expand_cond_ribs_with_at_least_one_rib ribs = 
      match ribs with
          | ScmNil -> ScmVoid
          | ScmPair (ScmPair (ScmSymbol "else", es), ScmNil) -> beginify es
          | ScmPair (ScmPair (expr, 
                              ScmPair (ScmSymbol "=>", 
                                      ScmPair (func_expr, ScmNil))), ribs) ->
              let continue = expand_cond_ribs_with_at_least_one_rib ribs in
              ScmPair
                (ScmSymbol "let",
                  ScmPair
                  (ScmPair
                    (ScmPair (ScmSymbol "e", ScmPair (expr, ScmNil)),
                      ...
                      (ScmPair
                        (ScmSymbol "fth",
                          ...
                          (ScmPair
                            (ScmSymbol "lambda",
                              ScmPair (ScmNil, ScmPair (func_expr, ScmNil))),
                            ScmNil)),
                        ScmPair
                        (ScmPair
                          (ScmSymbol "rest",
          ...
          | ScmPair (ScmPair (test, es), ribs) ->
            let dit = beginify es in
            let dif = expand_cond_ribs_with_at_least_one_rib ribs in
            ScmPair
              (ScmSymbol "if", 
        ...
        (* Ensure there's at least one rib *)
      let macro_expand_cond_ribs ribs =
        match ribs with
          | ScmNil -> raise (X_syntax "At least one rib is required in a cond expression")
          | _ -> expand_cond_ribs_with_at_least_one_rib ribs;;
  ...
  let rec modify_ribs ribs = 
    match ribs with 
      | ScmNil -> ScmNil
      | ScmPair (ScmPair (f, ScmPair (value, ScmNil)), restbindings) -> 
        let rest = modify_ribs restbindings in
        ScmPair (ScmPair (f, ScmPair (ScmPair (ScmSymbol "quote", ScmPair (ScmSymbol "whatever", ScmNil)), ScmNil)), rest)
      | _ -> raise (X_syntax "Invalid letrec rib format (modify_ribs)");;

  let rec create_bangs ribs exprs = 
    match ribs with
      | ScmPair (ScmPair (f, ScmPair (value, ScmNil)), restbindings) ->
            let rest = create_bangs restbindings exprs in
            ScmPair (
              ScmPair (ScmSymbol "set!",
                  ScmPair (f, ScmPair (value, ScmNil))), rest)
      | ScmNil -> exprs
      | _ -> raise (X_syntax "Invalid letrec format (create_bangs_and_let)");;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmSymbol var ->
       ...
       then (print_endline var; raise (X_syntax "Variable cannot be a reserved word"))
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      let new_ribs = modify_ribs ribs in
      let set_bangs_and_new_body = create_bangs ribs exprs in
        tag_parse 
          (ScmPair
            (ScmSymbol "let", ScmPair (new_ribs, set_bangs_and_new_body)))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address pe =
    ...
    let rec run expr params env =
      match expr with
        ...
        | ScmVarGet (Var var) -> ScmVarGet' (tag_lexical_address_for_var var params env)
        | ScmIf (expr1, expr2, expr3) ->  let test = run expr1 params env in
                                          let dit = run expr2 params env in
                                          let dif = run expr3 params env in
                                          ScmIf' (test, dit, dif)                         
        ...
        | ScmVarSet (Var var, expr) ->  let annotated_var = tag_lexical_address_for_var var params env in
                                        let annotated_expr = run expr params env in
                                        ScmVarSet' (annotated_var, annotated_expr)
        | ScmVarDef (Var var, expr) ->  let annotated_var = tag_lexical_address_for_var var params env in
                                        let annotated_expr = run expr params env in
                                        ScmVarDef' (annotated_var, annotated_expr)
        | ScmLambda (new_params, Simple, expr) ->   let extended_env = params :: env in
                                                    let annotated_expr = run expr new_params extended_env in
                                                    ScmLambda' (new_params, Simple, annotated_expr)
        | ScmLambda (new_params, Opt opt, expr) ->  let extended_env = params :: env in
                                                    let annotated_expr = run expr (new_params @ [opt]) extended_env in
                                                    ScmLambda' (new_params, Opt opt, annotated_expr)
        | ScmApplic (proc, exprs)-> let annotated_expr = run proc params env in
                                    let annotated_exprs = List.map (fun expr -> run expr params env) exprs in
                                    ScmApplic' (annotated_expr, annotated_exprs, Non_Tail_Call) 
  ...
  let annotate_tail_calls pe = 
    (*raise (X_not_yet_implemented "hw 2");;*)
    let rec run is_tail_p = function
      | ScmConst' sexpr ->  ScmConst' sexpr
      | ScmVarGet' (var') -> ScmVarGet' (var')
      | ScmIf' (expr1', expr2', expr3') -> let test = run false expr1' in
                                        let dit = run is_tail_p expr2'  in
                                        let dif  = run is_tail_p expr3'  in
                                        ScmIf'(test, dit, dif)
      | ScmSeq' (exprs') -> let rec set_tail_value = function 
                                  | expr' :: [] -> [run is_tail_p expr']
                                  | expr' :: exprs' -> (run false expr') :: (set_tail_value exprs')
                                  | _ -> raise (X_this_should_not_happen "ScmSeq is empty.")
                            in ScmSeq' (set_tail_value exprs')
    
      | ScmOr' (exprs') -> let rec set_tail_value = function 
                                  | expr' :: [] -> [run is_tail_p expr']
                                  | expr' :: exprs' -> (run false expr') :: (set_tail_value exprs')
                                  | _ -> raise (X_this_should_not_happen "ScmOr is empty.")
                            in ScmOr' (set_tail_value exprs')
                            (*Changed here!!!*)
      | ScmVarSet' (var', expr') -> ScmVarSet'(var',run false expr')
      | ScmVarDef' (var', expr') -> let expr_tailed = run false expr' in
                                      ScmVarDef' (var', expr_tailed)
      | ScmLambda' (params', lambda_kind, expr') -> let expr_tailed = run true expr' in
                                                    ScmLambda' (params', lambda_kind, expr_tailed)
      | ScmApplic' (proc, exprs', app_kind) ->  let tailed_proc = run false proc in
                                                let tailed_exprs = List.map (fun expr' -> run false expr') exprs' in 
                                                 (match is_tail_p with
                                                    | true -> ScmApplic' (tailed_proc, tailed_exprs, Tail_Call)
                                                    | false -> ScmApplic' (tailed_proc, tailed_exprs, Non_Tail_Call))
      | _ -> raise (X_this_should_not_happen "annotate_tail_calls : Need to add boxes?")
      in run false pe;;
  (* auto_box *)
...
module Code_Generation : CODE_GENERATION = struct
  ...
  let collect_free_vars pe =
    ...
    let rec run table = function
      | [] -> table
      | ScmConst' (sexpr) :: rest -> run table rest    
      | ScmVarGet' (Var' (name, Free)) :: rest -> [name] @ (run table rest)
      | ScmVarGet' (var') :: rest -> run table rest
      | ScmIf'(test, dit, dif) :: rest -> (run table [test]) @ (run table [dit]) @ (run table [dif]) @ (run table rest)
      | ScmSeq'(exprs') :: rest -> (run table exprs') @ (run table rest)
      | ScmOr'(exprs') :: rest -> (run table exprs') @ (run table rest)
      | ScmVarSet'(Var' (name, Free), expr') :: rest-> [name] @ (run table [expr']) @ (run table rest)
      | ScmVarSet'(var', expr') :: rest -> (run table [expr']) @ (run table rest)
      | ScmVarDef'(Var' (name, Free), expr') :: rest-> [name] @ (run table [expr']) @ (run table rest)
      | ScmVarDef'(var', expr') :: rest -> (run table [expr']) @ (run table rest)
      | ScmBox'(Var' (name, Free)) :: rest -> [name] @ (run table rest)
      | ScmBox'(var') :: rest -> run table rest
      | ScmBoxGet'(Var' (name, Free)) :: rest -> [name] @ (run table rest)
      | ScmBoxGet'(var') :: rest -> run table rest
      | ScmBoxSet'(Var' (name, Free), expr') :: rest-> [name] @ (run table [expr']) @ (run table rest)
      | ScmBoxSet'(var', expr') :: rest -> (run table [expr']) @ (run table rest)
      | ScmLambda'(params, lambda_kind, expr') :: rest -> (run table [expr']) @ (run table rest)
      | ScmApplic'(expr', exprs', app_kind) :: rest -> (run table [expr']) @ (run table exprs') @ (run table rest)

  in 
     let free_vars = run [] pe in
     let globals =
          List.map
            (fun (scheme_name, _) -> scheme_name)
            global_bindings_table
  in
    remove_duplicates (globals @ free_vars);;
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmIf' (test, dit, dif) ->
         ...
         let l_exit = make_if_end () in
         let test_gen = run params env test in
         let test_asm_code = 
          test_gen
          ^ "\tcmp rax, sob_boolean_false\n"
          ^ (Printf.sprintf "\tje %s\n" l_else) in
         let dit_gen = run params env dit in
         let dit_asm_code =
         dit_gen
         ^ (Printf.sprintf "\tjmp %s\n" l_exit)
         ^ (Printf.sprintf "%s:\n" l_else) in
         let dif_gen = run params env dif in
         let dif_asm_code =
         dif_gen
         ^ (Printf.sprintf "%s:\n" l_exit) in
         test_asm_code
      ...
      | ScmVarSet' (Var' (v, Free), expr') ->
         (*raise (X_not_yet_implemented "final project")*)
         let label = search_free_var_table v free_vars in
         let expr_gen = run params env expr' in
      ...
      | ScmBoxSet' (var', expr') ->
         (* raise (X_not_yet_implemented "final project") *)
         let expr_gen = run params env expr' in
         let var_gen = run params env (ScmVarGet' var') in
         let expr_asm_code = 
          expr_gen
          ^ "\tpush rax\n" in
          let var_asm_code =
      ...
      | ScmLambda' (params', Simple, body) ->
         ...
         in
         (*Allocate memory for closure, 1-T_closure (data type), 8-pointer for env, 8-pointer for code*)
         "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
         ^ "\tcall malloc\n"
         ^ "\tpush rax\n"
         (*Stack: ptr_to_closure*)
         (*Allocate memory for new array in env[0] (the size is num of params)*)
         ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; new rib\n" params)
         ^ "\tcall malloc\n"
         ^ "\tpush rax\n"
         (*RAX - Allocate memory for the extended env. It is +1 of the old env.*)
         ^ (Printf.sprintf "\tmov rdi, 8 * %d\t; extended env\n" (env + 1))
         ^ "\tcall malloc\n"
         (*pointer to old env*)
         ^ "\tmov rdi, ENV\n"
         (*rsi - index of curr env array. rsi=env we finished copying*)
         ^ "\tmov rsi, 0\n"
         (*rdx - index of the new env array we wnat to copy to. ext_env[i+1]*)
         ^ "\tmov rdx, 1\n"
         (*label of begining of copy env loop*)
         ^ (Printf.sprintf "%s:\t; ext_env[i + 1] <-- env[i]\n"
              label_loop_env)
         (*Check if we finished copying*)
         ^ (Printf.sprintf "\tcmp rsi, %d\n" env)
         (*If finished copying, exit the copy loop*)
         ^ (Printf.sprintf "\tje %s\n" label_loop_env_end)
         (*We are here if we want to continue the copy*)
          (*rcx = pointer to the current array of old env*)
         ^ "\tmov rcx, qword [rdi + 8 * rsi]\n"
         (*copy the pointer of the old env to the new env (but in higher index)*)
         ^ "\tmov qword [rax + 8 * rdx], rcx\n"
         (*Inc indexs*)
         ^ "\tinc rsi\n"
         ^ "\tinc rdx\n"
         (*Jump back to the next copy*)
         ^ (Printf.sprintf "\tjmp %s\n" label_loop_env)
         (*label to the end of the copy of env.*)
         ^ (Printf.sprintf "%s:\n" label_loop_env_end)
         (*If we are here, it means ext_env is almost ready, ext_env[0] not yet*)
         (*rbx = pointer to the ext_env[0]*)
         ^ "\tpop rbx\n"
         (*rsi = index of parameter*)
         ^ "\tmov rsi, 0\n"
         ^ (Printf.sprintf "%s:\t; copy params\n" label_loop_params)
         ^ (Printf.sprintf "\tcmp rsi, %d\n" params)
         ^ (Printf.sprintf "\tje %s\n" label_loop_params_end)
         (*rdx = get parameter[rsi]*)
         ^ "\tmov rdx, qword [rbp + 8 * rsi + 8 * 4]\n"
         (*copy parameter to the ext_env[0][rsi]*)
         ^ "\tmov qword [rbx + 8 * rsi], rdx\n"
         (*inc index*)
         ^ "\tinc rsi\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_loop_params)
         ^ (Printf.sprintf "%s:\n" label_loop_params_end)
         (*If we are here the ext_env is ready!*)
         (*rax = ext_env, rbx = ext_env[0]*)
         ^ "\tmov qword [rax], rbx\t; ext_env[0] <-- new_rib \n"
         (*rbx = pointer to the ext_env*)
         ^ "\tmov rbx, rax\n"
         (*rax = pointer to closure*)
         ^ "\tpop rax\n"
         (*THIS IS WHAT THE FIRST BYTE IS FOR, mark that this is closure type*)
         ^ "\tmov byte [rax], T_closure\n"
         (*Set closure env to ext_env*)
         ^ "\tmov SOB_CLOSURE_ENV(rax), rbx\n"
         (*Set closure code to label_code*)
         ^ (Printf.sprintf "\tmov SOB_CLOSURE_CODE(rax), %s\n" label_code)
         (*jmp Lcont*)
         ^ (Printf.sprintf "\tjmp %s\n" label_end)
         (*Here we are in the lambda body - Lcode:*)
         ^ (Printf.sprintf "%s:\t; lambda-simple body\n" label_code)
         (*Note for me: params' are for our lambda list of parameters,
             params is for the previous frame NUMBER of parameters (only for ext_env[0])*)
         (*rsp is pointing to return address that was pushed before entering the lambda*)
         (*we check that number of parameters is correct.*)
         ^ (Printf.sprintf "\tcmp qword [rsp + 8 * 2], %d\n"
           (List.length params'))
         (*If number of parameters is ok, we jump to good code, else we continue to error*)
         ^ (Printf.sprintf "\tje %s\n" label_arity_ok)
         ^ "\tpush qword [rsp + 8 * 2]\n"
         ^ (Printf.sprintf "\tpush %d\n" (List.length params'))
         ^ "\tjmp L_error_incorrect_arity_simple\n"
         ^ (Printf.sprintf "%s:\n" label_arity_ok)
         (*If we are here, the number of parameters is ok.*)
         (*enter 0,0 is some kind of 
            push rbp
            mov rbp, rsp?*)
         ^ "\tenter 0, 0\n"
         (*code generation of the lambda body.*)
         ^ (run (List.length params') (env + 1) body)
         (*the opposite of enter, 
            mov rsp, rbp
            pop rbp = the old frame pointer*)
         ^ "\tleave\n"
         (*we return to the old frame and kill(clean) the current one.*)
         ^ (Printf.sprintf "\tret AND_KILL_FRAME(%d)\n" (List.length params'))
         (*End of code of closure.*)
      ...
      | ScmLambda' (params', Opt opt, body) ->
         ...
         and label_exectly_shrink_loop = make_lambda_opt_loop ()
         and label_exectly_push_ret = make_lambda_opt_loop_exit ()
         and label_arity_more = make_lambda_opt_arity_more ()
         and label_list_build_loop = make_lambda_opt_loop ()
         and label_change_arg_num_value = make_lambda_opt_loop ()
         and label_stack_shrink_loop = make_lambda_opt_loop ()
         and label_start_pop_loop_op = make_lambda_opt_loop ()
        ...
        in
         ...
         (*rax = pointer to closure*)
         ^ "\tpop rax\n"
         (*THIS IS WHAT THE FIRST Bit IS FOR, mark that this is closure type*)
         ^ "\tmov byte [rax], T_closure\n"
         (*Set closure env to ext_env*)
         ^ "\tmov SOB_CLOSURE_ENV(rax), rbx\n"
         (*Set closure code to label_code*)
         ...
         ^ (Printf.sprintf "%s:\n" label_arity_more)
         (*Here we handle the case when there ARE optional arguments.*)
          (* calculate optional arguments size *)
          (* --------------------------------------------- *)

          (* gets size of n in stack *)
         ^ "\tmov r8, qword [rsp + 8 * 2]\n"
          (* gets size of non optional parametrs *)
          ^(Printf.sprintf "\tmov r9 , %d\n" (List.length params'))
          (* we know that r8 > r9 *)
          (* Subtract the value of 'r9' from r8 *)
          ^"\tmov r15, rsp\n"
          ^"\tsub r8, r9\n"
          
          (* difference is stored in the r8  *)
          (* loop for creating the list at the top  *)
          (* ---------------------------------------------- *)
          (* counter reg *)
          ^"\tmov r9, 0\n"
          (* rbx point to top *)
          ...
          ^"\timul rax, 8\n"
          ^"\tadd rbx, rax\n"
          (* save value of first val to reg *)
          ^"\tmov rdx , qword [rbx]\n"
          (* save pointer to the top *)
          ^"\tmov r10 , rbx\n"
          (*Allocate memory for pair, 1-T_pair (data type), 8-pointer for car, 8-pointer for cdr*)
         ^ "\tmov rdi, (1 + 8 + 8)\t; sob pair\n"
         (*Save the registers before malloc call*)
         ^"\tpush rdx\n"
         ^"\tpush rbx\n"
         ^"\tpush r8\n"
         ^"\tpush r9\n"
         ...
         ^"\tsub rbx, 8\n"
         ^"\tcmp r9, r8\n"
         ^(Printf.sprintf "\tje %s\n" label_change_arg_num_value)

         ^(Printf.sprintf "%s:\n" label_list_build_loop)
         ^"\tmov rdx, qword [rbx]\n"
         ^"\tpush rbx\n"
         ^"\tpush r8\n"
         ^"\tpush r9\n"
         ...
         ^(Printf.sprintf "\tjl %s\n" label_list_build_loop)
          ...
          (* move the values (optional_size - 1) cells up *)
          (* -------------------------------- *)
          
          (* 3 lines that change the num of arguments(non optional + 1) *)
          (*NOTE: label_change_arg_num_value is not a loop but we had to create a label*)
          ^(Printf.sprintf "%s:\n" label_change_arg_num_value)
          ^"\tlea rdx, [r15 + 8 * 2]\n"
          ^(Printf.sprintf "\tmov qword [rdx], %d\n" (List.length params'))
          ^"\tadd qword [rdx] , 1\n"
          (* rbx point to the highest non optional arg *)
          (* r8 = size of optional arg *)
          ^"\tmov r9, r8\n"
          (* calculate how many cells to move up: *)
          ^"\tdec r9\n"
          (* r9 points to the cell we need to move the rbx *)
          ^"\tlea r9, [rbx + 8 * r9]\n"
          ...
          ^"\tsub r9 , 8\n"
          ^"\tsub rbx, 8\n"
          (* while rbx is not points to RET *)
          ^"\tcmp rbx, r15\n"
          ^(Printf.sprintf "\tjne %s\n" label_stack_shrink_loop)
          ^"\tmov rax, qword [rbx]\n"
          ^"\tmov qword [r9], rax\n"
          (* if rbx point to RET needs to pop (size of optional arg -1) *)
          (* create a counter *)
          ^"\tmov r9, r8\n"
          ^"\tsub r9 , 1\n"
          (*----------------------------------------------------------------------------*)
          (*No need to pop - handel case where there are 1 arg 1 opt*)
          ^"\tcmp r9, 0\n"
          ^(Printf.sprintf "je %s\n" label_stack_ok)
          (*----------------------------------------------------------------------------*)
          ^(Printf.sprintf "%s:\n" label_start_pop_loop_op)
          (*Just pop the trash*)
          ^"\tpop rax\n"
          ^"\tdec r9\n"
          ^"\tcmp r9, 0\n"
          ^(Printf.sprintf "\tjg %s\n" label_start_pop_loop_op)
          (* finish with pop stack is ok *)
          ^(Printf.sprintf "jmp %s\n" label_stack_ok)
          ^(Printf.sprintf "%s:\n" label_arity_exact)
         (*Here we handle the case when there is no optional argument.*)
         (*rbx saves the pointer to the current cell*)
         ^"\tmov r15, rsp\n"
         ^ (Printf.sprintf "\tlea rbx, [r15 + 8 * 2 + 8 * %d]\n" (List.length params'))
         (*rcx is a temp for next value to insert to cell*)
         ^ "\tmov rcx, qword [rbx]\n"
         ^ "\tmov qword [rbx], sob_nil\n"
         ^ "\tsub rbx, 8\n"
         
         (*rdx is a temp for the waiting value*)
         ^ (Printf.sprintf "%s:\n" label_exectly_shrink_loop)
         ^ "\tmov rdx, qword [rbx]\n"
         ^ "\tmov qword [rbx], rcx\n"
         ^ "\tcmp rbx, r15\n"
         ^ (Printf.sprintf "je %s\n" label_exectly_push_ret)
         ^ "\tmov rcx, rdx\n"
         ^ "\tsub rbx, 8\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_exectly_shrink_loop)
         ^ (Printf.sprintf "%s:\n" label_exectly_push_ret)
         ...
         ^ "\tenter 0, 0\n"
         (*code generation of the lambda body.*)
         (*WE ADD +1 FOR THE OPTIONAL PARAMETER*)
